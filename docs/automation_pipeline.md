...
5. Successful runs update `state/daily_scanner_state.json` with the git hash and Mongo `_id`, preventing duplicates on future scans. Failed steps stay out of the state file so the next run will retry automatically.
